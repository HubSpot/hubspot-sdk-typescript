...
describe('resource complete', () => {
  ...
  test.skip('create: only required params', async () => {
    const responsePromise = client.automation.v4.actions.callbacks.complete.create({
  ...
  test.skip('create: required and optional params', async () => {
    const response = await client.automation.v4.actions.callbacks.complete.create({
      inputs: [{ callbackId: 'callbackId', outputFields: { foo: 'string' } }],
    });
  });
});
