...
describe('resource associate', () => {
  // Prism tests are disabled
  test.skip('createDefault: only required params', async () => {
    const responsePromise = client.crm.v4.associations.batch.associate.createDefault('toObjectType', {
  ...
  test.skip('createDefault: required and optional params', async () => {
    const response = await client.crm.v4.associations.batch.associate.createDefault('toObjectType', {
      fromObjectType: 'fromObjectType',
      inputs: [{ from: { id: 'id' }, to: { id: 'id' } }],
    });
  });
