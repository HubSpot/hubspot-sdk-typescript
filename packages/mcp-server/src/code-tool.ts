...
import { ClientOptions } from '@hubspot/sdk';

const prompt = `Runs JavaScript code to interact with the HubSpot API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = { HUBSPOT_BASE_URL: readEnv('HUBSPOT_BASE_URL') ?? client.baseURL ?? undefined };
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      project_name: 'hubspot-sdk',
      code,
      intent,
      client_opts: {
        accessToken: readEnv('HUBSPOT_ACCESS_TOKEN'),
        developerAPIKey: readEnv('HUBSPOT_DEVELOPER_API_KEY'),
      },
  ...
  try {
    const resp = await new Promise<Response>((resolve, reject) => {
      ...
      const opts = {
        ...
        ...(client.developerAPIKey != null ? { developerAPIKey: client.developerAPIKey } : undefined),
        defaultHeaders: {
