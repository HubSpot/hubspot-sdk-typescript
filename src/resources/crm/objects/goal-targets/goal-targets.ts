...
export class GoalTargets extends APIResource {
   ...
   *   await client.crm.objects.goalTargets.create({
   *     properties: { foo: 'string' },
   *   });
   * ```
   */
  create(
    body: GoalTargetCreateParams,
    options?: RequestOptions,
  ): APIPromise<CRMAPI.CreatedResponseSimplePublicObject> {
    return this._client.post('/crm/v3/objects/goal_targets', { body, ...options });
  }
   ...
   *   await client.crm.objects.goalTargets.update(
   *     'goalTargetId',
   *     {
   *       properties: {
  ...
  update(
    ...
    return this._client.patch(path`/crm/v3/objects/goal_targets/${goalTargetID}`, {
      query: { idProperty },
      body,
      ...options,
    });
  ...
  delete(goalTargetID: string, options?: RequestOptions): APIPromise<void> {
    return this._client.delete(path`/crm/v3/objects/goal_targets/${goalTargetID}`, {
      ...options,
   ...
   *   await client.crm.objects.goalTargets.get('goalTargetId');
   * ```
   */
  get(
    goalTargetID: string,
    ...
    return this._client.get(path`/crm/v3/objects/goal_targets/${goalTargetID}`, { query, ...options });
  }
   ...
   *   await client.crm.objects.goalTargets.search();
   * ```
   */
  search(
    ...
    return this._client.post('/crm/v3/objects/goal_targets/search', { body, ...options });
  }
}
