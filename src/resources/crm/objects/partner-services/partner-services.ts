...
export class PartnerServices extends APIResource {
   ...
   *   await client.crm.objects.partnerServices.update(
   *     'partnerServiceId',
  ...
  update(
    ...
    return this._client.patch(path`/crm/v3/objects/partner_services/${partnerServiceID}`, {
      query: { idProperty },
      body,
      ...options,
    });
   ...
   * for await (const simplePublicObjectWithAssociations of client.crm.objects.partnerServices.list()) {
   *   // ...
   * }
   * ```
   */
  list(
    query: PartnerServiceListParams | null | undefined = {},
    options?: RequestOptions,
   ...
   *   await client.crm.objects.partnerServices.get(
   *     'partnerServiceId',
   *   );
   * ```
   */
  get(
    ...
    return this._client.get(path`/crm/v3/objects/partner_services/${partnerServiceID}`, {
      query,
      ...options,
    });
  }
  ...
  search(
    ...
    return this._client.post('/crm/v3/objects/partner_services/search', { body, ...options });
  }
}
