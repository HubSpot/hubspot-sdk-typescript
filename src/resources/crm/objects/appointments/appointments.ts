...
export class Appointments extends APIResource {
   ...
   *   await client.crm.objects.appointments.create({
   *     properties: { foo: 'string' },
   *   });
   * ```
   */
  create(
    body: AppointmentCreateParams,
    options?: RequestOptions,
  ): APIPromise<CRMAPI.CreatedResponseSimplePublicObject> {
    return this._client.post('/crm/v3/objects/0-421', { body, ...options });
  }
   ...
   *   await client.crm.objects.appointments.update(
   *     'appointmentId',
   *     {
   *       properties: {
  ...
  update(
    ...
    return this._client.patch(path`/crm/v3/objects/0-421/${appointmentID}`, {
      query: { idProperty },
      body,
      ...options,
    });
   ...
   * for await (const simplePublicObjectWithAssociations of client.crm.objects.appointments.list()) {
   *   // ...
   * }
   * ```
   */
  list(
    ...
    return this._client.getAPIList('/crm/v3/objects/0-421', Page<CRMAPI.SimplePublicObjectWithAssociations>, {
      query,
      ...options,
    });
  }
   ...
   * @example
   * ```ts
   * await client.crm.objects.appointments.delete(
   *   'appointmentId',
   * );
   * ```
   */
  delete(appointmentID: string, options?: RequestOptions): APIPromise<void> {
    return this._client.delete(path`/crm/v3/objects/0-421/${appointmentID}`, {
      ...options,
   ...
   *   await client.crm.objects.appointments.get(
   *     'appointmentId',
   *   );
   * ```
   */
  get(
    appointmentID: string,
    query: AppointmentGetParams | null | undefined = {},
    options?: RequestOptions,
  ): APIPromise<CRMAPI.SimplePublicObjectWithAssociations> {
    return this._client.get(path`/crm/v3/objects/0-421/${appointmentID}`, { query, ...options });
  }
   ...
   *   await client.crm.objects.appointments.search();
   * ```
   */
  search(
    body: AppointmentSearchParams,
    options?: RequestOptions,
  ): APIPromise<CRMAPI.CollectionResponseWithTotalSimplePublicObject> {
    return this._client.post('/crm/v3/objects/0-421/search', { body, ...options });
  }
}
