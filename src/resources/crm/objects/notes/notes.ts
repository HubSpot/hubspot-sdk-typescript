...
export class Notes extends APIResource {
   ...
   *   await client.crm.objects.notes.create({
   ...
   *   await client.crm.objects.notes.update('noteId', {
   *     properties: { foo: 'string' },
   *   });
   * ```
   */
  ...
  delete(noteID: string, options?: RequestOptions): APIPromise<void> {
    return this._client.delete(path`/crm/v3/objects/notes/${noteID}`, {
      ...options,
   ...
   *   await client.crm.objects.notes.get('noteId');
   * ```
   */
  get(
    noteID: string,
   ...
   *   await client.crm.objects.notes.search();
   * ```
   */
  search(
